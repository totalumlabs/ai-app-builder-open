...
export function LogsPanel({ projectId }: LogsPanelProps) {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    if (res.ok && res.data) {
      ...
      const message = res.error || "Failed to fetch logs. The server might not be active.";
      setError(message);
      setLogs("");
    }
    setFetched(true);
    ...
    <div className="h-full flex flex-col">
      <div className="px-3 py-2 border-b bg-gray-50/80 dark:bg-gray-900/40 dark:border-gray-800 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-xs font-medium text-gray-500 truncate">{"Server Logs"}</span>
        </div>
        <Button
          variant="ghost"
          size="icon"
        ...
        {!fetched ? (
          <div className="flex flex-col items-center justify-center h-full gap-4">
            ...
            <div className="text-center max-w-xs">
              ...
              <p className="text-xs text-gray-600 mb-4">{"Live server logs — one unified stream covering both the preview and your published production site."}</p>
            </div>
            <Button
              variant="secondary"
              size="sm"
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3 text-center px-4">
            ...
            <p className="text-sm text-red-300 font-medium">{"Failed to fetch logs. The server might not be active."}</p>
          ...
          <div className="flex flex-col items-center justify-center h-full gap-2 text-center px-4">
            ...
            <p className="text-xs text-gray-500 max-w-xs">{"No logs available yet. Interact with your app (e.g. submit a form) and refresh to see new entries."}</p>
          </div>
        )}
      </div>
    </div>
