...
export default function WorkspacePage() {
  ...
  useEffect(() => {
    ...
    const handleMove = (e: MouseEvent) => {
      ...
      const newWidth = Math.max(280, Math.min(600, resizeRef.current.startWidth + (e.clientX - resizeRef.current.startX)));
    ...
    <div className="h-screen flex flex-col overflow-hidden" style={{ background: "#fcfbf8" }}>
      ...
      <header className="h-11 flex items-center px-2 shrink-0 bg-white border-b border-gray-100 z-10">
        ...
        <div className="flex items-center gap-1">
          ...
          {/* Collapse button inline in header - positioned at aside right edge */}
          <button
            onClick={() => setChatCollapsed(!chatCollapsed)}
            className="hidden sm:flex p-1.5 rounded-md text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors"
          >
            {chatCollapsed ? <PanelLeft className="w-4 h-4" /> : <PanelLeftClose className="w-4 h-4" />}
          </button>
          <div className="w-px h-5 bg-gray-200 mx-1 hidden sm:block" />
          <h1 className="text-sm font-medium text-gray-700 truncate max-w-[120px]">{projectId}</h1>
          <span className={`w-1.5 h-1.5 rounded-full shrink-0 ml-1 ${getServerStatusColor()}`} />
        </div>

        {/* Center: Tabs in header */}
        ...
        <div className="flex items-center gap-1">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="p-1.5 rounded-md text-gray-400 hover:text-gray-700 hover:bg-gray-100"><MoreVertical className="w-4 h-4" /></button>
            </DropdownMenuTrigger>
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Chat panel - desktop */}
        <div
          className={`hidden sm:flex flex-col shrink-0 bg-white transition-all ${chatCollapsed ? "w-0 overflow-hidden" : ""}`}
        ...
        {/* Right panel - no borders, just margin for seamless look */}
